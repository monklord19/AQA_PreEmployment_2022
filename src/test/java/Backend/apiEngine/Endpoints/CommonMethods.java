...
public class CommonMethods {
    ...
    public static String setResponseAsString(@NotNull Response response) {
        String responseAsString = response.asPrettyString();
        return responseAsString;
    }

    public static @NotNull ValidatableResponse validateStatusCode(@NotNull Response response, Integer statusCode) {
        ValidatableResponse validateResponse = response.then();
        validateResponse.statusCode(Matchers.equalTo(statusCode));
        return validateResponse;
    }

    public static @NotNull ValidatableResponse validateResponseTime(@NotNull Response response, Long responseTime) {
        ValidatableResponse validateResponse = response.then();
        validateResponse.time(Matchers.lessThanOrEqualTo(responseTime));
        return validateResponse;
    }

    public static @NotNull ValidatableResponse validateBodyNotNull(@NotNull Response response) {
        ValidatableResponse validatableResponse = response.then();
        validatableResponse.assertThat().body(Matchers.notNullValue());
        return validatableResponse;
    }

    public static @NotNull ValidatableResponse validateValuesFromResponseBody(@NotNull Response response, String keyName, String keyValue) {
        ValidatableResponse validatableResponse = response.then();
