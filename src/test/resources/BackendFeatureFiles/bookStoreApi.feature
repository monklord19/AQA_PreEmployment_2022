...
Feature: BookStore/v1 endpoint
  ...
  Scenario: POST /Account/v1/User and POST /Account/v1/GenerateToken
    Given send a POST request for registration of new user with username: 'SRadu1070' and password: 'Tast@tura1234'
  ...
  Scenario: GET /BookStore/v1/Books
    Given send a GET request for list of books
    Then number of books is: 8

  Scenario: GET /BookStore/v1/Book
    Given send a GET request for book no.3
  ...
  Scenario: POST /BookStore/v1/Books
    Given send a POST request for adding book no.3 on users list
  ...
  Scenario: DELETE /BookStore/v1/Book
    Given send a DELETE request for book no.1 from user list
  ...
  Scenario: PUT /BookStore/v1/Books/{ISBN}
    Given send a PUT request for replacing book no.1 from user books list with book no.6 from book store books list
  ...
  Scenario: DELETE /BookStore/v1/Books
    Given send a DELETE request for all books from user list
    When send a GET request for account details for userID
